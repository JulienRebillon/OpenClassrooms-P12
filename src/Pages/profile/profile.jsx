    ...
    useEffect(() => {
        if (userId) {
            if (useMock) {
                ...
                const rawPerformanceData = extractData(USER_PERFORMANCE.find(user => user.userId === parseInt(userId)));
                console.log("Mock USER_PERFORMANCE data:", rawPerformanceData); // Log the raw data

                // Test with known mock data for performance
                const testPerformanceData = [
                    { kind: "Cardio", value: 120 },
                    { kind: "Energy", value: 110 },
                    { kind: "Endurance", value: 130 },
                    { kind: "Strength", value: 90 },
                    { kind: "Speed", value: 100 },
                    { kind: "Intensity", value: 80 },
                ];
                setPerformanceData(testPerformanceData); // Set the test data
            } else {
                const fetchData = async () => {
                    try {
                        const user = await fetchUserData(userId);
